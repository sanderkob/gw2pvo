...
# pvo.add_status(data['pgrid_w'], last_eday_wh, cons_wh, cons_w, data.get('temperature'), voltage, data['temperature'], paneltemp, data['vpv1'], data['vpv2'], data['Ppv1'], data['Ppv2'])
    def add_status(self, SunUp, pgrid_w, eday_wh, cons_wh, cons_w, temperature, voltage, invertertemp, paneltemp, vpv1, vpv2, Ppv1, Ppv2,v2,v4):
        t = time.localtime()        
        if SunUp:
            payload = {
            ...
            if paneltemp is not None:
                payload['v5'] = paneltemp
...
# second upload according to https://gathering.tweakers.net/forum/list_message/75283144#75283144
        payload = {
            'd' : payload["d"],
            't' : payload["t"],
            'n' : 1,            
